...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/pardisontop/pardis-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    download_link="https://raw.githubusercontent.com/pardisontop/pardis-ui/master/install.sh"
...
uninstall() {
    ...
    systemctl disable pardis-ui
    rm /etc/systemd/system/pardis-ui.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/pardis-ui/ -rf
    rm /usr/local/pardis-ui/ -rf
    echo -e "\nUninstalled Successfully."
    echo ""
    echo -e "If you need to install this panel again, you can use below command:"
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/pardisontop/pardis-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    echo -e "${green} Please use the new login username and password to access the pardis-ui panel. Also remember them! ${plain}"
...
reset_webbasepath() {
    ...
    /usr/local/pardis-ui/pardis-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/pardis-ui/pardis-ui setting -reset
...
check_config() {
    info=$(/usr/local/pardis-ui/pardis-ui setting -show true)
    if [[ $? != 0 ]]; then
...
get_uri() {
    info=$(/usr/local/pardis-ui/pardis-ui uri)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/pardis-ui/pardis-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "pardis-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "pardis-ui and xray stopped successfully"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "pardis-ui and xray Restarted successfully"
    else
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "pardis-ui Set to boot automatically on startup successfully"
    else
        LOGE "pardis-ui Failed to set Autostart"
    fi
...
disable() {
    systemctl disable pardis-ui
    if [[ $? == 0 ]]; then
        LOGI "pardis-ui Autostart Cancelled successfully"
    else
        LOGE "pardis-ui Failed to cancel autostart"
    fi
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u pardis-ui -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
        before_show_menu
        fi
        ;;
...
update_shell() {
    wget -O /usr/bin/pardis-ui -N --no-check-certificate https://github.com/pardisontop/pardis-ui/raw/main/pardis-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/pardis-ui.service ]]; then
        return 2
    fi
    temp=$(systemctl status pardis-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled pardis-ui)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /usr/local/pardis-ui/pardis-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
show_usage() {
    ...
    echo "pardis-ui restart-xray - Restart xray-core"
...
show_menu() {
    echo -e "
  ${green}pardis-ui Admin Management Script ${plain}
————————————————
  ${green}0.${plain} Exit 
————————————————
