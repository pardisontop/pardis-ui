...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/alireza0/pardis-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    download_link="https://raw.githubusercontent.com/alireza0/pardis-ui/master/install.sh"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/alireza0/pardis-ui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/pardis-ui -N --no-check-certificate https://github.com/alireza0/pardis-ui/raw/main/pardis-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
